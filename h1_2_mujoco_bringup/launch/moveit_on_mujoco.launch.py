...
def load_yaml(package_name, relative_path):
    ...
    if not os.path.exists(absolute_path):
        print(f"[WARN] YAML not found: {absolute_path}")
        return {}
...
def generate_launch_description():
    moveit_pkg = "h1_2_moveit_config"
    mujoco_pkg = "h1_2_mujoco_bringup"

    robot_description_content = ParameterValue(
        Command([
            "xacro ",
            PathJoinSubstitution([
                FindPackageShare(mujoco_pkg),
                "description",
                "h1_2_mujoco.urdf.xacro",
            ]),
        ]),
        value_type=str,
    )
