...
class VisionNode(Node):
    def __init__(self):
        ...
        # [MUJOCO CAMERA VARIANT] Tópicos parametrizables para cámara MuJoCo
        self.declare_parameter('color_topic', '/camera/color')
        self.declare_parameter('depth_topic', '/camera/depth')
        self.declare_parameter('camera_info_topic', '/camera/camera_info')

        # [CUBE ORIENTATION FIX]
        # Corrección local de yaw aplicada a los ejes publicados del cubo.
        # No afecta la posición ni la normal usada para calcular el centro.
    ...
    def depth_to_meters(self, z_raw):
        ...
        # MuJoCo publica depth como 32FC1 en metros.
        if self.latest_depth_encoding == '32FC1':
            return z

        # Cámaras RGB-D reales suelen publicar 16UC1 en milímetros.
        if self.latest_depth_encoding in ('16UC1', 'mono16'):
            return z / 1000.0

        # Fallback seguro: valores grandes se interpretan como mm; valores pequeños como m.
        if z > 20.0:
            return z / 1000.0
        return z
                ...
                if cv2.contourArea(best_cnt) > 500:
                    ...
                    if len(approx) == 4:
                        ...
                        if len(pts_3d) > 15:
                            ...
                            # [CUBE ORIENTATION 3D CORNER FIX]
                            # En vez de usar solo el ángulo 2D TL->TR, proyectamos las esquinas
                            # detectadas sobre el plano 3D de la cara visible. Esto corrige el
                            # sesgo por perspectiva y mejora la lectura de yaw en MuJoCo.
                            ...
                            # [CUBE ORIENTATION FIX]
                            # Corrección local del frame del cubo. Post-multiplicación:
                            # conserva el eje Z/normal y solo rota X/Y del objeto.
